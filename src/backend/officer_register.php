...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $tableName = 'deiofficer';

    // Check if the user already exists in the database
    $sqlCheckExistingUser = "SELECT * FROM $tableName WHERE email = :email AND institute = :institute";
    $stmtCheckExistingUser = $connection->prepare($sqlCheckExistingUser);
    $stmtCheckExistingUser->bindValue(':email', $email, PDO::PARAM_STR);
    $stmtCheckExistingUser->bindValue(':institute', $institute, PDO::PARAM_INT);
    ...
    $sql = "INSERT INTO $tableName (name, email, phoneNumber, password, institute) VALUES (:name, :email, :phoneNumber, :password, :institute)";
